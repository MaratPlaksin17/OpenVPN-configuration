...
function installQuestions() {
	...
	echo "   1) Self-hosted DNS Resolver (Unbound)"

	until [[ $DNS =~ ^[0-9]+$ ]] && [ "$DNS" -ge 1 ] && [ "$DNS" -le 13 ]; do
		read -rp "DNS [1-1]: " -e -i 1 DNS
		if [[ $DNS == 1 ]] && [[ -e /etc/unbound/unbound.conf ]]; then
			echo ""
	...
	until [[ $CUSTOMIZE_ENC =~ (y|n) ]]; do
		read -rp "Customize encryption settings? [y/n]: " -e -i CUSTOMIZE_ENC
	done
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		CC_CIPHER="ECDHE-ECDSA-AES-256-GCM-SHA384"
		DH_TYPE="2" # DH
		DH_CURVE="secp521r1"
		HMAC_ALG="SHA-512"
		TLS_SIG="1" # tls-crypt
	else
	#выбираем наш канал данных
		echo ""
		echo "Choose which cipher you want to use for the data channel:"
		echo "   1) AES-256-GCM(recommended)"

		until [[ $CIPHER_CHOICE =~ ^[1-6]$ ]]; do
			read -rp "Cipher [1-1]: " -e -i 1 CIPHER_CHOICE
		done
		...
		2)
			...
			echo "   2) ECDHE-RSA-AES-256-GCM-SHA384(recommended)"
			until [[ $CC_CIPHER_CHOICE =~ ^[1-2]$ ]]; do
				read -rp"Control channel cipher [1-2]: " -e -i 2 CC_CIPHER_CHOICE
			done
			case $CC_CIPHER_CHOICE in
